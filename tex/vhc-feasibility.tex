...
\AtBeginDocument{
  \DeclareGraphicsExtensions{
    .pdf,.PDF,.eps,.EPS,.png,.PNG,.tif,.TIF,.jpg,.JPG,.jpeg,.JPEG
  }
}
...
% \paperfield{Control Theory}
% \corraddress{Author One PhD, Department, Institution, City, State or Province, Postal Code, Country}
\corremail{surov.m.o@gmail.com}
...
\author[1]{Maksim Surov}

\affil[1]{
  Maksim Surov is with Department of Information Technologies and AI, 
  Sirius University of Science and Technology, 
  Sochi, Russia
  {\tt\small surov.m.o@gmail.com}
}

\runningauthor{Maksim Surov}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    While the existing literature has established a widely accepted definition of VHCs, we argue that this definition is overly restrictive and excludes a broad class of admissible trajectories from consideration. To illustrate this point, we consider an example of periodic motion of the planar vertical take-off and landing system that satisfies all standard requirements, including orbital stabilizability. 
    However, for this solution -- and for many similar ones -- there exists no VHC that satisfies the conventional definition. We further provide a formal proof showing that the constraints imposed by the standard definition necessarily fail for a broad class of trajectories. Finally, we show that an alternative, less restrictive definition -- originally proposed in earlier works but rarely used in practice -- admits such trajectories and thus considerably extends the set of known solutions.
  \end{abstract}

  \section{Introduction}
    In this paper, we consider Euler–Lagrange systems of the form
    \begin{equation}
    \label{eq:lag-sys}
    ...
    with Jacobian $dh\left(q\right) := \frac{\partial h\left(q\right)}{\partial q}$
    ...
    The feasibility properties of the related concept of \emph{servo constraints} were also considered in~\cite{Chen-2009}. In Definition~3 therein, the author introduced the \emph{controllability} property of a servo constraint and, in Theorem~4, established conditions under which this property holds. These conditions do not imply the existence of a controlled invariant manifold $\Gamma$.

    In this work, we demonstrate that Definition~\ref{def:VHC} describes only a narrow subclass of VHCs and conflicts with other, more general interpretations. To support this claim, we begin our exposition in Section~\ref{sec:pvtol-trajectory} with an example of a periodic trajectory of the planar vertical take-off and landing (PVTOL) aircraft that satisfies standard motion planning requirements, including orbital stabilizability. For this trajectory, we demonstrate the absence of any VHC satisfying Definition~\ref{def:VHC}. However, by relaxing the requirement of controlled invariance of $\Gamma$, the VHC-based approach remains applicable for generating the trajectory and constructing a stabilizing feedback control. We show that this approach is coherent with existing more general interpretations of VHCs.
    Generalizing this observation to systems of type~\eqref{eq:lag-sys}, we formulate the main result of the paper in Theorem~\ref{thm:singularity-necessity}, which states that Definition~\ref{def:VHC} necessarily fails for a broad class of trajectories. Section~\ref{sec:VHC-definitions} provides a comparative analysis of various interpretations of VHCs found in the literature and shows that our results are coherent with the more general ones. Section~\ref{sec:pvtol-periodic-trajectories} demonstrates a set of new solutions of the PVTOL system that become available when a more general interpretation of VHCs is adopted. Finally, Section~\ref{sec:conclusion} offers concluding remarks.
  ...
  \subsection*{Notation}
    ...
    \begin{itemize}
      \item
        ...
        along a vector field $f\in \mathbb{R}^{n}$ is denoted by $L_{f}h\left(q\right) := \frac{\partial h\left(q\right)}{\partial q}f\left(q\right)\in\mathbb{R}^{k}$.
      \item
        The higher-order directional derivatives are defined recursively as:
        $L_{f}^{l}h\left(q\right) := L_{f}L_{f}^{l-1}h\left(q\right)\in\mathbb{R}^{k}$.
      \item
        The Lie bracket of two vector fields $f,g \in \mathbb{R}^{k}$ is defined as $\mathrm{ad}_{f} g := L_{f}g - L_{g}f$.
      \item
        The Lie bracket of higher order is defined recursievly as $\mathrm{ad}^l_f g := \mathrm{ad}_f (\mathrm{ad}^{l-1}_f g)$.
      \item
        The Jacobian matrix of a function $h:\mathbb{R}^{n}\to\mathbb{R}^{k}$
        is denoted by $dh\left(q\right) := \frac{\partial h\left(q\right)}{\partial q}\in\mathbb{R}^{k\times n}$.
      \item 
        The column space of a matrix $A$ is denoted by $\mathrm{Im}\left[A\right]$. 
      \item
        The kernel of a matrix $A\in\mathbb{R}^{n\times m}$ is defined as: $\ker\left[ A\right] := \left\{ x\in\mathbb{R}^{m}\mid Ax=0\right\}.$
    \end{itemize}

  \section{Motivating Example: Periodic Trajectory of PVTOL Aircraft}
  \label{sec:pvtol-trajectory}
    ...
    \label{proposition:no-vhc-for-pvtol}
      ...
      \begin{enumerate}
        \item 
          $h\left(q_{*}\left(t\right)\right) \equiv 0$ for all $t \in \mathbb{R}$,
        \item 
          $\mathrm{rank}\,dh\left(q\right) = 2$ for all $q \in h^{-1}\left(0\right)$,
        \item 
          the set
          \[
            \Gamma := \left\{ \left(q,\dot{q}\right)\in\mathbb{R}^{6}\mid h\left(q\right)=0
    ...
    \label{proof:proposition}
      ...
      \end{itemize}
      Consider the point $q_{0}\in\mathcal{C}$ given by:
      $$
      ...
      \end{align*}
      The condition $h\left(q_{*}\left(t\right)\right) \equiv 0$ implies
    ...
    \label{eq:implicit-vhc-candidate}
      h\left(q\right)=\left(\begin{array}{c}
        ...
        \end{array}\right)
    \end{equation}
    or any other smooth function $h(q)$ that satisfies requirements~1) and~2) of Proposition~\ref{proposition:no-vhc-for-pvtol}.
    While the constraint~\eqref{eq:implicit-vhc-candidate} does not meet Definition~\ref{def:VHC}, it is straightforward to show that it satisfies the conditions of Theorem~4 in~\cite{Chen-2009} and is therefore a controllable servo constraint.
  ...
  \label{sec:pendubot-trajectory-stabilizability}
    ...
    \[
      \Psi(q,\dot q) := \left[f,g_{1},g_{2},\mathrm{ad}_{f}g_{1},\mathrm{ad}_{f}g_{2},\mathrm{ad}_{f}^{2}g_{1}\right]
    \]
    which equals to
    \[
  ...
  \subsection{Synthesis of Orbitally Stabilizing Feedback}
    ...
    \]
    The variables $\rho := \left(\rho_{1},\dots,\rho_{5}\right)^{\top}$
    ...
    and solve for $\dot{\rho}$ and $\dot{\tau}$. Since the transformation is diffeomorphic, the Jacobian matrix is invertible, ensuring the validity of this step. Moreover, we observe that $\dot \tau > 0$ within a tubular neighborhood and bounded input $w$, which justifies -- following~\cite{Banaszuk-1995,Finet-2015} -- dividing $\dot{\rho}$ by $\dot{\tau}$ to obtain the transverse dynamics in the form of a general nonlinear system:
    \begin{equation}
    \label{eq:transverse-dynamics-2}
      \frac{d\rho}{d\tau}=A\left(\tau\right)\rho+B\left(\tau\right)w+o\left(\tau,\rho,w\right),
    \end{equation}
    where
    $A(\tau) \in \mathbb{R}^{5\times5}$ and 
    $B(\tau) \in \mathbb{R}^{5\times2}$ are $2\pi$-periodic matrix functions and
    $o\left(\tau,\rho,w\right)$ collects bilinear, quadratic and higher-order terms in $\rho$ and $w$.

    Based on the arguments in~\cite{Yakubovich-1986}, the controllability of the LTV system
    \begin{equation}
    \label{eq:linearized-transverse-dynamics}
      \frac{d\bar{\rho}}{d\tau} = A\left(\tau\right)\bar{\rho}+B\left(\tau\right)w
    \end{equation}
  ...
  \subsection{Numerical Analysis}
    ...
    \[
      \mathrm{eigvals} \, W\left(0,2\pi\right): \quad \left\{ 744.0,\,70.7,\,15.3,\,5.16,\,0.0537\right\},
    \]
    ...
    \begin{figure}\begin{centering}
        \includegraphics[width=0.99\linewidth]{fig/pvtol_phase_coords}
      \par
      \caption{Closed loop system simulation results.}
      \label{fig:pvtol-sim}
    \end{centering}\end{figure}
    \begin{figure}\begin{centering}
        \includegraphics[width=0.99\linewidth]{fig/pvtol_transverse}
      \par
      \caption{Evolution of transverse coordinates and control inputs versus trajectory projection $\tau$ in the closed loop system.}
      \label{fig:pvtol-transverse}
    \end{centering}\end{figure}
  ...
  \label{sec:non-regular-vhc}
    ...
    existence theorem. However, as shown in~\cite[Theorem~1]{Surov-2025} and~\cite[Theorem~4]{Remizov-2023}, smooth solutions to second-order differential equations with isolated singularities may still exist, provided that certain additional conditions are satisfied by the coefficients $\alpha(\theta)$, $\beta(\theta)$, and $\gamma(\theta)$.
    We now recall the formulation of this existence
    \begin{thm}
    \label{thm:singular-solutions-existence}
      Consider the differential
      equation~(\ref{eq:alpha-beta-gamma}) with coefficients 
      ...
      \begin{itemize}
        ...
        \item
          ...
          \]
      \end{itemize}
      For any real values $\theta_{1},\theta_{2}\in I$, $\theta_{1}<\theta_{s}<\theta_{2}$,
    ...
    \label{thm:singularity-necessity} 
      Let $q_{*}\left(t\right)$ be a smooth trajectory of the system~(\ref{eq:lag-sys}). Suppose that, at some time $t_{s}$, the trajectory satisfies 
      \begin{align}
      ...
      \end{align}
      where $q_{s} := q_{*}\left(t_{s}\right)$ and $B_{\perp}\left(q\right)\in\mathbb{R}^{1\times n}\setminus\left\{ 0\right\}$  is a left annihilator of $B\left(q\right)$. 
      Then for any smooth parametrization of the form $q_{*}\left(t\right)=\phi\left(\theta_{*}\left(t\right)\right)$, the associated reduced dynamics necessarily satisfy
      \[
        \alpha \left(\theta_{s}\right) = 0, \quad \text{where} \quad
        \theta_{s} := \theta_{*}\left(t_{s}\right).
      \]
    ...
    \begin{proof}
      ...
      \[
        \Gamma := \left\{ \left(q,\dot{q}\right)\in\mathbb{R}^{2n}\mid h\left(q\right)=0\quad \text{and} \quad L_{\dot{q}}h\left(q\right)=0\right\}
      \]
      is controlled invariant. This implies that the following system of linear algebraic equations in $u$ must be solvable:
      \begin{align*}
        & dh\left(q\right)M^{-1}\left(q\right)\left(B\left(q\right)u-C\left(q,\dot{q}\right)\dot{q}-G\left(q\right)\right)+L_{\dot{q}}^{2}h = 0,
      \end{align*}
  ...
  \label{sec:VHC-definitions}
    To the best of our knowledge, Definition~\ref{def:VHC} was first formally introduced in~\cite{Jankuloski-2012}, although the requirement for the controlled invariance of $\Gamma$ appeared in earlier works such as~\cite{Consolini-2011,Consolini-2010}. 
    ...
    The concept of servo-constraints, described in~\cite{Chen-2009}, also assumes a more general comprehension. The author formulating

    The author does not require invariant properties of a manifold, instead they consider the restrictions on the control input
  ...
  \begin{thebibliography}{10}
    ...
    \bibitem{Surov-2025}
       M. Surov, Singular motion generators in the problem of motion planning for underactuated mechanical systems,
       Nonlinear Dynamics 113, 29697--29718 (2025).
    ...
    \bibitem{Surov-2025}
      ...
      Regul. Chaot. Dyn. 30, 451–462 (2025),
      doi: 10.1134/S1560354725030013.

    \bibitem{Chen-2009}
      Chen Y-H.,
      Constraint-following Servo Control Design for Mechanical Systems,
      Journal of Vibration and Control, 2009;15(3):369--389.

    \bibitem{Finet-2015}
      S. Finet L. Praly,
      Feedback Linearization of the Transverse Dynamics for a Class of One Degree Underactuated Systems,
      Proc. of the 54th IEEE Conference on Decision and Control,
      Japan, pp. 7802--7807, 2015.

    \bibitem{Remizov-2023}
      A. O. Remizov, Singularities of quasi-linear differential equations, 2023, issue 1, pp. 85--105, doi: 10.47910/FEMJ202308.
