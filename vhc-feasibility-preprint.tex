...
\begin{document}
  ...
  \section{Introduction}
    ...
    This system is underactuated with an underactuation degree of one.
    
    For the system~(\ref{eq:lag-sys}), we consider the problem of motion
    ...
    which ensures that the planned motion can be realized and maintained on a physical system.
    ...
    the VHC framework enables reduction of the original $2n$-dimensional
    ...
    to this equation automatically defines a corresponding trajectory
    $q_{*}\left(t\right)=\phi\left(\theta_{*}\left(t\right)\right)$ of
    the original system~(\ref{eq:lag-sys}). This technique has significantly
    ...
    \end{defn}
    %
    Similar definitions can be found in other publications \cite{Consolini-2011,Otsason-2019,Consolini-2010,Jankuloski-2012,Consolini-2018}
    by the same group of authors. In some of these works \cite{Consolini-2011,Consolini-2010,Jankuloski-2012},
    the term \textit{feasible} VHC is used. A common requirement
    ...
    invariant is overly restrictive} in the context of motion planning,
    as it excludes a broad class of viable solutions. While the existence
    of the structure of the controlled invariant manifold 
    may be beneficial in certain applications -- such
    as the design of stabilizing feedback controllers -- it is by no
    means a necessary condition. In particular, the lack of controlled
    invariance does not necessarily imply that trajectory stabilizability
    is impossible. Therefore, we find the usage of the term \textit{feasible}
    to describe only a narrow class of VHCs somewhat misleading. If one
    follows this terminology, it would imply that a ``\textit{non-feasible}''
    VHC could nevertheless be used to generate feasible trajectories --
  ...
  %       along a vector field $f\in T\mathbb{R}^{n}$ is denoted by $L_{f}h\left(q\right)\equiv\frac{\partial h\left(q\right)}{\partial q}f\left(q\right)\in\mathbb{R}^{k}$.
  %     \item
  ...
  \subsection{Tic-Toc Maneuver of PVTOL Aircraft}
    Let us consider two functions $q_{*}\left(t\right)\in C^{\omega}$, $u_{*}\left(t\right)\in C^{\omega}$ defined as
    \begin{align}
    \label{eq:ref-traj}
  ...
  \subsection{The Absence of Controlled Invariance}
    ...
    $\dot{q}$, namely $\dot{q}=\pm\dot{q}_{0}$. However, if we consider $\dot{q}=0\in T_{q_{0}}\mathcal{Q}$, the second term of~(\ref{eq:dq_constr}) vanishes, causing the equality to be violated.
    This contradicts the requirement that~\eqref{eq:dq_constr} must hold for all $\dot{q} \in T_{q_{0}} \mathcal{Q}$.
    ...
    However, this requirement is a direct consequence of imposing a VHC that constrains the system to move along a prescribed direction. In real-world scenarios, slight deviations from the VHC allow the system to traverse the corresponding configuration $q_{0}$ with a range of possible velocities.

    The existence of a stabilizing feedback for the trajectory~(\ref{eq:ref-traj}) follows directly from~\cite[Theorem4]{Nam-1992}, as the dimension of the corresponding Lie algebra for the PVTOL aircraft equals 6 for almost all points along the reference orbit.
    Alternatively, one can design a state feedback controller using the transverse linearization approach with the transverse coordinates defined as in~\cite{Surov-2020}.

    In this article, we restrict ourselves to presenting the results of computer simulation, available at~\url{https://youtube.com/shorts/EM2qfht1D74}, where we used the transverse coordinates $\tau \in [-\pi, \pi), \rho \in \mathbb{R}^5$, defined as
    \begin{align*}
    ...
    \end{align*}
    with the VHC $h(q)$ as in~(\ref{eq:implicit-vhc-candidate}), together with the new control input
    $
  ...
  \label{sec:non-regular-vhc}
    ...
    and consequently, the trajectory $q_{*}\left(t\right)$ presented in~(\ref{eq:ref-traj}).
    ...
    We now briefly recall the formulation of this existence
    \begin{thm}
    \label{thm:singular-solutions-existence}
      Let us consider the differential
      equation~(\ref{eq:alpha-beta-gamma}) with the coefficients 
      $\alpha(\theta), \beta(\theta)$ and $\gamma(\theta)$ defined on an open interval $\theta\in I\subset\mathbb{R}$.
    ...
    \label{rem:periodic-solitions}
      ...
      The function $\theta_{*}\left(-t\right)$
    ...
    \label{thm:singularity-necessity} 
      ...
      \label{eq:singularity-condition}
        \exists t_{s} & :\,\dot{q}_{*}\left(t_{s}\right)\ne0\quad\text{and}\nonumber \\
      ...
      is a left annihilator of $B\left(q\right)$. Then the reduced dynamics~(\ref{eq:alpha-beta-gamma})
    ...
    The singularities in the reduced dynamics play a crucial role in the loss of controlled invariance of the set $\Gamma$. 
    For a trajectory $q_{*}\left(t\right)$ of a general mechanical system~(\ref{eq:lag-sys}) the vanishing
  of the coefficient $\alpha\left(\theta_{s}\right)$
    ...
    reduces the system~(\ref{eq:condition-for-dq}) to a quadratic equation in $\dot{\theta}_{s}$:
    \[
    ...
    \subsection*{Example: PVTOL Aircraft Trajectories}
      ...
      to dynamics~(\ref{eq:alpha-beta-gamma-poly-vhc}), we obtain the following system of inequalities:
      \begin{align*}
      ...
      \end{align*}
      This system is solvable\footnote{one can find other solutions, multiplying all the coefficients of the reduced dynamics by $-1$.} on the interval $0<\psi_{s}<\pi$ under the conditions $k_{3}<0$ and $k_{1}k_{2}>-k_{3}$.
      Thus, based on Remark~\ref{rem:periodic-solitions}, we conclude the existence of periodic solutions. 
      In particular, the PVTOL aircraft can perform a periodic motion with small amplitude around a configuration with attitude $\psi_s = \frac{\pi}{4}$.
      Simulation results for such a trajectory are available at~\url{https://youtube.com/shorts/EM2qfht1D74}. One can further prove, using arguments similar to those presented above, that all such trajectories also cannot be found when adopting Definition~\ref{def:VHC}.

  \section{Concluding Remarks}
  \label{sec:conclusion}
    We have demonstrated, through a specific example, that there exists a class of feasible solutions for mechanical systems that satisfy all conventional conditions except for the controlled invariance of the manifold $\Gamma$ in Definition~\ref{def:VHC}. Moreover, we have shown that a VHC which does not satisfy this definition can still serve to define transverse coordinates and enable the construction of stabilizing feedback.

    The PVTOL system is not unique in this regard. By applying Theorem~\ref{thm:singular-solutions-existence}, one can construct similar trajectories for other underactuated systems such as the Pendubot, the Furuta pendulum (as in~\cite{Surov-2018}), the cart-pendulum, and others. As demonstrated, these solutions do not admit the existence of a controlled invariant manifold $\Gamma$.

    Therefore, we conclude that the requirement of controlled invariance for $\Gamma$ is superfluous. In our view, the feasibility of a VHC of the form $h(q) = 0$ with $dh(q)$ of full rank on $\mathcal{C}$, is determined by the existence of a solution $(q_*(t), u_*(t))$ to the system~(\ref{eq:lag-sys}) defined on a nontrivial time interval $I$, such that $h(q_*(t)) \equiv 0$ for all $t \in I$. 
    
    Furthermore, a single, uniform definition of VHC may be unnecessarily restrictive, as various applications may demand different properties. For instance, the requirement of continuous differentiability of $h(q)$ over the entire set $\mathcal{C}$ may be too strong in scenarios involving hybrid systems. It is, therefore, more appropriate to tailor the definition and requirements of VHCs to the specifics of each application. A comprehensive analysis and classification of such requirements is an important direction for future research.
